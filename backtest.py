#!/usr/bin/env python3
"""
backtest.py — TradingAgents NSE/BSE (India) market backtester (OpenAI)

Usage:
    python backtest.py

Edit BACKTEST_CONFIG below to set tickers, date range, models, and frequency.
Use .NS suffix for NSE tickers (e.g. RELIANCE.NS) or .BO for BSE.
...
BACKTEST_CONFIG = {
    ...
    # Analysis frequency:
    #   "weekly"  → first Indian trading day of each calendar week  (recommended)
    #   "monthly" → first Indian trading day of each calendar month (cheapest)
    #   "daily"   → every Indian trading day                        (expensive)
    "frequency": "weekly",
...
def generate_trading_dates(start: str, end: str, frequency: str) -> list[str]:
    """Return Mon-Fri trading dates in [start, end] at the requested frequency."""
